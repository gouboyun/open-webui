...
export const addFileToPdfById = async (token: string, id: string|null, fileId: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/pdffolder/${id}/file/add`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const deletePdfAll = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/pdffolder/all`, {
		method: 'DELETE',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
